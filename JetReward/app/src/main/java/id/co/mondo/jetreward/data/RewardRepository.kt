package id.co.mondo.jetreward.data

import id.co.mondo.jetreward.model.FakeRewardDataSource
...
class RewardRepository {
    ...
    fun updateOrderReward(rewardId: Long, newCountValue: Int): Flow<Boolean>{
        ...
        val result = if (index >= 0) {
            ...
            orderRewards[index] = orderReward.copy(
                reward = orderReward.reward, count = newCountValue
            )
            true
        }else{
            false
    ...
    fun getAddedOrderRewards(): Flow<List<OrderReward>>{
        return getAllRewards()
            .map { orderRewards ->
                orderRewards.filter { orderRewards ->
                    orderRewards.count != 0
                }
            }
    }
